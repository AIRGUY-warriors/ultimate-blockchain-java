...
public class ABlockchain implements Blockchain {
    ...
    protected void createNewBlock() {
        ...
        Block newBlock = new Block(currentBlock.getBlockHash(), //O(1)
                                 transactionsPerBlock, blockCounter++);
        chain.insertRear(newBlock);//O(B),where B is the number of blocks in the chain
        // also insert into blocksTree
        blocksTree.insert(newBlock);//O(logB)
        currentBlock = newBlock; //O(1)
    }//This method is O(B)

    protected void processBlockTransactions() {
        // Execute transactions and update balances for current block
        TransactionWithFee[] transactions = currentBlock.getTransactions();//O(logT*T),where T is the number of transactions in block
        for (TransactionWithFee transaction : transactions) {//O(T),where T is the number of transactions in block
            String from = transaction.getFromAddress();//O(1)
            String to = transaction.getToAddress();//O(1)
            int amount = transaction.getAmount();//O(1)
            int fromBalance = balance.getBalance(from);//O(1)
            int fee = transaction.getFee();//O(1)
            if (fromBalance >= amount + fee) {
                successfulTransactionsCount++;//O(1)
                // Execute transaction
                balance.updateBalance(from, fromBalance - amount - fee);//Max(O(L),O(A))
                int toBalance = balance.getBalance(to);//O(L)
                balance.updateBalance(to, toBalance + amount);//Max(O(L),O(A))
                balance.updateBalance("0", balance.getBalance("0") + fee); // collect fee,Max(O(L),O(A))
            } else {
                revertedTransactionsCount++;//O(1)
                // Revert transaction
                if(fromBalance >= fee/2) {
                    balance.updateBalance(from, fromBalance - fee/2);//Max(O(L),O(A))
                    balance.updateBalance("0", balance.getBalance("0") + fee/2); // collect fee/2,//Max(O(L),O(A))
                    returnedFees += fee/2;//O(1)
                }
                transaction.revert();//O(1)
            }
        }
    }//The complexity analysis here is considering the method in UBlockChain.java
     //Let's denote the max(A,L) = A&L,and Let's denot the max(A&L,T)=M, then the complexity of the funciton is
     // O(logT*M)
	...
	public boolean repOK() {
        ...
        Block[] blocks = getBlocks().toArray();//O(B),B is the number of blocks
        for (Block block : blocks) {//O(B)
            if (block.getTransactionCount() > transactionsPerBlock) {
                return false;//O(1)
            }
        }
        // 2) Check: balances are non-negative
        for (String address : balance.getAllAddresses()) {//O(A),where A is the number of addresses
            if (balance.getBalance(address) < 0) {
                return false;//O(1)
            }
        }
        // 3) Check: transaction pool is a valid max-heap
        transactionPool.repOK();//O(TP),where TP is the number of transactions in the pool

        return true;//O(1)
    
    }//Denote the max{TP,A,B} = M2, then the complexity is O(M2)
