...
public class ListoverLinkedList<T> implements Sequence<T> {
    ...
    public T[] toArray() {
        ...
        Node<T> curr = head;
        int i = 0;
        while (curr != null) {//O(n),where n is the size of the sequence
            arr[i] = curr.data;
            curr = curr.next;
            i++;
        }
        return arr;
    }//this method is complexity O(n)
    ...
    }//This method is O(n),where n is the number of element in the sequence
